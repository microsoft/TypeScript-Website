import { PlaygroundConfig } from "./startPlayground";
      ...
      if (relativePath) {
        ...
        if (newPath) {
          ...
          if (!dtsReferenceResponse.ok) {
            return errorMsg(`Could not get ${newPath} for a reference link in the module '${mod}' from ${path}`, dtsReferenceResponse, config);
          }

          let dtsReferenceResponseText = await dtsReferenceResponse.text();
          if (!dtsReferenceResponseText) {
            return errorMsg(`Could not get ${newPath} for a reference link for the module '${mod}' from ${path}`, dtsReferenceResponse, config);
          }
...
const getDependenciesForModule = (
  ...
  filteredModulesToLookAt.forEach(async name => {
    ...
    if (isPackageRootImport) {
      ...
      const absolutePathForModule = mapRelativePath(moduleName, moduleToDownload, path);
