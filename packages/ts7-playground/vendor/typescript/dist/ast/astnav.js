...
import { createSyntaxList, createToken, } from "./factory.generated.js";
...
const emptyArray = [];
function assertHasRealPosition(node) {
...
export function getChildren(node, sourceFile = node.getSourceFile()) {
    ...
    const cache = (sourceFile.childrenCache ??= new Map());
...
function addSyntheticNodes(children, pos, end, parent, sourceFile, scanner) {
    if (pos >= end) {
        return;
    }
    scanner.resetTokenState(pos);
...
function createSyntaxListNode(nodes, parent, sourceFile, scanner) {
    const listChildren = [];
    let pos = nodes.pos;
    ...
    const list = createSyntaxList(listChildren);
    list.pos = nodes.pos;
    list.end = nodes.end;
    list.parent = parent;
    return list;
}
export function getFirstToken(node, sourceFile = node.getSourceFile()) {
    if (isTokenKind(node.kind)) {
        return undefined;
    }
    assertHasRealPosition(node);
...
export function getLastToken(node, sourceFile = node.getSourceFile()) {
    if (isTokenKind(node.kind)) {
        return undefined;
    }
    assertHasRealPosition(node);
