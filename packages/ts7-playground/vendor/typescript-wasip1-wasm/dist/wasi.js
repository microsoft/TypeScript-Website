...
function createWasiHost(options) {
    let instance;
    ...
    function argsSizesGet(countPointer, sizePointer) {
        countPointer >>>= 0;
        sizePointer >>>= 0;
        const view = getView();
    ...
    function clockTimeGet(clockId, _precision, timePointer) {
        timePointer >>>= 0;
        let nanoseconds;
        switch (clockId) {
            case 0:
    ...
    function fdWrite(fd, iovsPointer, iovsLength, writtenPointer) {
        iovsPointer >>>= 0;
        writtenPointer >>>= 0;
    ...
    function hostWriteFile(iovsPointer, iovsLength, writtenPointer) {
        ...
        if (12 + pathLength + dataLength + errorCapacity !== bufferLength)
            return errnoInvalidArgument;
    ...
    function pollOneoff(subscriptionsPointer, eventsPointer, subscriptionsLength, eventsLengthPointer) {
        ...
        for (let i = 0; i < subscriptionsLength; i++) {
            ...
            if (view.getUint8(subscription + 8) !== eventTypeClock)
                continue;
