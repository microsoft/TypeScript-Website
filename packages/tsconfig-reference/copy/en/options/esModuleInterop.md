...
TypeScript uses compatibility helpers for CommonJS interop. Before `esModuleInterop` was enabled by default, TypeScript treated CommonJS modules similar to ES modules in ways that turned out to be flawed assumptions:
...
`esModuleInterop` fixes both of these problems in the code transpiled by TypeScript. The first changes the behavior in the compiler, the second is fixed by two helper functions which provide a shim to ensure compatibility in the emitted JavaScript:
