...
import { deprecated, internal, defaultsForOptions, allowedValues, configToRelease, } from "../tsconfigRules.js";
...
// @ts-ignore
const allFlags = ts.optionDeclarations.concat(notCompilerFlags);
...
const strippedOpts = filteredOptions.filter((opt) => !buildOpts.includes(opt.name) && !watchOpts.includes(opt.name));
