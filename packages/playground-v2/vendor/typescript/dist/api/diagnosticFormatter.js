import { convertToRelativePath } from "./path.js";
...
function diagnosticPrefix(diagnostic) {
    return diagnostic.source || "TS";
}
function flattenDiagnosticMessage(diagnostic, newLine, indentLevel = 0) {
    let result = "";
    if (indentLevel) {
...
function relativeFileName(fileName, host) {
    return convertToRelativePath(fileName, host.getCurrentDirectory(), name => host.getCanonicalFileName(name));
}
function formatLocation(diagnostic, host) {
...
function formatCodeSpan(diagnostic, lineIndent, squiggleColor, host) {
    const { startPosition, endPosition, sourceLines } = diagnostic;
    if (!startPosition || !endPosition || !sourceLines?.length)
        return "";
