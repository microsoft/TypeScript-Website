...
export class RemoteSourceFile extends RemoteNode {
    ...
    get spanMap() {
        if (this._spanMapRead)
            return this._cachedSpanMap;
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (tupleLength !== 5 && tupleLength !== 6)
                throw new Error("Invalid span map segment");
            ...
            const features = tupleLength === 6 ? reader.readUint() : SpanMapFeature.All;
            if (kind !== SpanMapKind.Verbatim && kind !== SpanMapKind.Atom && kind !== SpanMapKind.Alias)
                throw new Error(`Invalid span map kind: ${kind}`);
            segments[i] = {
                virtualStart,
    ...
    get supplementalSourceFileNames() {
        if (this._cachedSupplementalSourceFileNames !== undefined)
            return this._cachedSupplementalSourceFileNames;
    ...
    get diagnosticDirectives() {
        if (this._diagnosticDirectivesRead)
            return this._cachedDiagnosticDirectives;
        ...
        const reader = new MsgpackReader(buf, this._offsetStructuredData + offset);
        const count = reader.readArrayHeader();
        const directives = Array(count);
        for (let i = 0; i < count; i++) {
            if (reader.readArrayHeader() !== 6)
                throw new Error("Invalid diagnostic directive");
            const originalStart = reader.readUint();
            const originalLength = reader.readUint();
            const virtualStart = reader.readUint();
            const virtualLength = reader.readUint();
