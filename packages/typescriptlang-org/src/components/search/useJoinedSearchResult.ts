import { useEffect, useState } from "react"

import { joinSearchResults } from "./joinSearchResults";
import { JoinedSearchResult, RawSearchResults } from "./types";

const createRequestData = (search: string) => ({
    analyticsTags: ["yarnpkg.com"],
    attributesToHighlight: ["name", "description", "keywords"],
    attributesToRetrieve: [
        "deprecated",
        "description",
        "downloadsLast30Days",
        "homepage",
        "humanDownloadsLast30Days",
        "keywords",
        "license",
        "modified",
        "name",
        "owner",
        "repository",
        "types",
        "version",
    ],
    facets: ["keywords", "keywords", "owner.name"],
    hitsPerPage: 51,
    indexName: "npm-search",
    maxValuesPerFacet: 10,
    page: 0,
    params: "",
    query: search,
    tagFilters: "",
})

const createPostData = (search: string) => {
    const searches = search.startsWith("@types/")
        ? [search.substring("@types/".length), search]
        : [search, `@types/${search}`]

    return {
        requests: searches.map(createRequestData)
    }
}

const searchParams = new URLSearchParams({
    "x-algolia-agent": "TS DT Fetch",
    "x-algolia-application-id": "OFCNCOG2CU",
    "x-algolia-api-key": "f54e21fa3a2a0160595bb058179bfb1e",
})

const href = `https://ofcncog2cu-2.algolianet.com/1/indexes/*/queries?${searchParams.toString()}`

export const useSearchResult = (search: string) => {
    const [result, setResult] = useState<JoinedSearchResult>()

    useEffect(() => {
        if (!search) {
            setResult(undefined)
            return
        }

        let stillValid = true

        fetch(href, { method: "POST", body: JSON.stringify(createPostData(search)) }).then(
            async response => {
                if (!stillValid) {
                    return
                }

                const json = await response.json()
                if (!stillValid) {
                    return
                }

                setResult(joinSearchResults((json as RawSearchResults).results))
            }
        )

        return () => {
            stillValid = false
        }
    }, [search])

    return result
}
