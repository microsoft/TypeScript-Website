...
const usagePercents = async () => {
  const versions = await get(
    "https://api.npmjs.org/versions/typescript/last-week"
  )
  const { downloads } = JSON.parse(versions)
  /** @type {Map<string, number>} */
  const mapVersions = new Map()
  Object.keys(downloads).forEach(key => {
    const mapKey = key.slice(0, 3)
    const oldValue = mapVersions.get(mapKey) || 0
    mapVersions.set(mapKey, oldValue + downloads[key])
  })

  let total = 0
  let usage = Array.from(mapVersions).reduce((obj, [key, value]) => {
    obj[key] = { count: value }
    total += value
    return obj
  }, {})
  ...
  Object.keys(usage).forEach(key => {
    usage[key].percent = Math.round(100 * (usage[key].count / total))
  })
...
const go = async () => {
  ...
  const usage = await usagePercents()
  const results = {
    "_generated by":
