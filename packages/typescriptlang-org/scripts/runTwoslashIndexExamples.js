...
const go = async () => {
  ...
  for (const path of readdirSync(examplesPath, "utf-8")) {
    ...
    const markdownAST = remark().parse("```tsx twoslash\n" + code + "\n```")
    await runTwoSlash(
      { markdownAST },
    ...
    const innerHTML = html.trim().slice(28, -6) // strips the pre
    const file = `// Auto-generated by yarn workspace typescriptlang-org compile-index-examples
import React from "react"

const innerHTML = \`${innerHTML}\`

export const ${componentName}Example = () => <pre className='shiki twoslash' dangerouslySetInnerHTML={{ __html: innerHTML }} />
`
