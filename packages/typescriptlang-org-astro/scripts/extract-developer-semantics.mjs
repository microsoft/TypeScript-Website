...
const sourceRoot = path.resolve(targetRoot, "../typescriptlang-org")
const sourcePublic = path.join(sourceRoot, "public")
const pages = [
  { route: "/branding", source: "src/pages/branding.tsx" },
  { route: "/dev/bug-workbench", source: "src/pages/dev/bug-workbench.tsx" },
  { route: "/dev/playground-plugins", source: "src/pages/dev/playground-plugins.tsx" },
  { route: "/dev/sandbox", source: "src/pages/dev/sandbox.tsx" },
  { route: "/dev/twoslash", source: "src/pages/dev/twoslash.tsx" },
  { route: "/dev/typescript-vfs", source: "src/pages/dev/typescript-vfs.tsx" },
]
...
const htmlPath = route => path.join(sourcePublic, route.replace(/^\//, ""), "index.html")
const content = {}
const copiedAssets = new Set()

for (const page of pages) {
  const sourcePage = path.join(sourceRoot, page.source)
  const baselinePage = htmlPath(page.route)
  if (!fs.existsSync(sourcePage)) throw new Error(`Missing Gatsby developer source page: ${sourcePage}`)
  if (!fs.existsSync(baselinePage)) throw new Error(`Missing Gatsby developer production baseline: ${baselinePage}`)
  const tokens = semanticTokens(fs.readFileSync(baselinePage, "utf8"))
  content[page.route] = { source: page.source, tokens }
  for (const token of tokens) {
    ...
    if (!fs.existsSync(source)) throw new Error(`Missing developer image asset: ${token.src}`)
...
const output = path.join(targetRoot, "src", "data", "developer-semantics.json")
fs.mkdirSync(path.dirname(output), { recursive: true })
fs.writeFileSync(output, `${JSON.stringify({
  generatedFrom: "Gatsby production main regions for the six inventoried filesystem/developer source pages",
  routeCount: pages.length,
  sourcePages: pages,
  copiedAssets: [...copiedAssets].sort(),
  content,
}, null, 2)}\n`)
console.log(`Extracted ${pages.length} developer routes and ${copiedAssets.size} referenced assets to target-owned data.`)
