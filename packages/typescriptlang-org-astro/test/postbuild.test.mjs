...
test(
  "emits every source redirect as a static fallback and an Azure permanent redirect",
  ...
  () => {
    ...
    assert.deepEqual(
      Object.fromEntries([
        ...redirectDeclarations.reduce(
          (counts, redirect) =>
            counts.set(
              redirectDestinationType(redirect.to),
              (counts.get(redirectDestinationType(redirect.to)) || 0) + 1
            ),
          new Map()
        ),
      ]),
    ...
    for (const redirect of redirectDeclarations) {
      ...
      assert.deepEqual(
        config.routes.find(rule => rule.route === redirect.from),
        { route: redirect.from, redirect: redirect.to, statusCode: 301 }
      )
      ...
      if (redirectDestinationType(redirect.to) !== "external") {
        ...
        const destinationOutput = new URL(
          destinationPath.endsWith(".html") ? destinationPath : `${destinationPath.replace(/\/$/, "")}/index.html`,
          dist
        )
        const destinationHtml = fs.readFileSync(destinationOutput, "utf8")
        if (destination.hash)
          assert.match(destinationHtml, new RegExp(`id=["']${destination.hash.slice(1)}["']`), redirect.to)
      }
    }
  }
)
...
test(
  "emits the complete application, redirect, error, and license HTML contract",
  { skip: !fs.existsSync(dist) },
  () => {
    ...
    assert.ok(walk(dist).length >= 506)
...
test(
  "keeps TSConfig legacy anchor compatibility while linking quick-nav to canonical targets",
  { skip: !fs.existsSync(dist) },
  () => {
    const tsconfig = fs.readFileSync(new URL("tsconfig/index.html", dist), "utf8")
    ...
    assert.match(
      tsconfig,
      /<span id="watchFile" aria-hidden="true"><\/span><span id="watchfile" aria-hidden="true"><\/span><span id="watch-file" aria-hidden="true"><\/span><h3 id='watch-watchFile-config'>/
    )
    assert.match(
      tsconfig,
      /<span id="disableFilenameBasedTypeAcquisition" aria-hidden="true"><\/span><span id="disablefilenamebasedtypeacquisition" aria-hidden="true"><\/span><span id="disable-filename-based-type-acquisition" aria-hidden="true"><\/span><h3 id='type-disableFilenameBasedTypeAcquisition-config'>/
    )
...
test(
  "all TSConfig option HTML pages are indexable and carry exact human redirects",
  { skip: !fs.existsSync(dist) },
  () => {
    const optionFiles = fs
      .readdirSync(new URL("tsconfig/", dist), { withFileTypes: true })
...
test("rewrites stale local handbook links to equivalent live routes", { skip: !fs.existsSync(dist) }, () => {
  const koBabelPath = new URL("ko/docs/handbook/babel-with-typescript.html", dist)
  if (fs.existsSync(koBabelPath)) {
    const koBabel = fs.readFileSync(koBabelPath, "utf8")
  ...
  const frHandbookIntroPath = new URL("fr/docs/handbook/intro.html", dist)
  if (fs.existsSync(frHandbookIntroPath)) {
    const frHandbookIntro = fs.readFileSync(frHandbookIntroPath, "utf8")
...
test(
  "preserves legacy fragment compatibility on docs, glossary, and localized pages",
  { skip: !fs.existsSync(dist) },
  () => {
    const modulesGuide = fs.readFileSync(
      new URL("docs/handbook/modules/guides/choosing-compiler-options.html", dist),
      "utf8"
    )
    assert.match(
      modulesGuide,
      /<span id="im-compiling-and-running-the-outputs-in-node" aria-hidden="true"><\/span><h3 id="im-compiling-and-running-the-outputs-in-nodejs"/
    )
    ...
    assert.match(
      modulesTheory,
      /<span id="module-specifiers-are-not-transformed" aria-hidden="true"><\/span><h3 id="module-specifiers-are-not-transformed-by-default"/
    )
    assert.match(
      modulesTheory,
      /<span id="extension-searching-and-directory-index-files" aria-hidden="true"><\/span><h3 id="module-resolution-for-libraries"/
    )
    assert.match(
      modulesTheory,
      /href="#extension-searching-and-directory-index-files">disables extension searching<\/a>/
    )
    ...
    const localizedAssertions = [
      [
        "id/docs/handbook/decorators.html",
        /<span id="class-decorators" aria-hidden="true"><\/span><h2 id="decorator-kelas"/,
      ],
      [
        "id/docs/handbook/decorators.html",
        /<span id="accessor-decorators" aria-hidden="true"><\/span><h2 id="decorator-aksesor"/,
      ],
      ["pt/docs/handbook/module-resolution.html", /<span id="classico" aria-hidden="true"><\/span><h3 id="clássico"/],
      [
        "pt/docs/handbook/module-resolution.html",
        /<span id="importacoes-de-modulos-relativos-vs.-nao-relativos" aria-hidden="true"><\/span><h2 id="importações-de-módulos-relativos-vs-não-relativos"/,
      ],
      [
        "ko/docs/handbook/decorators.html",
        /<span id="데코레이터-팩토리-DecoratorFactories" aria-hidden="true"><\/span><span id="데코레이터-팩토리-Decorator-Factories" aria-hidden="true"><\/span><h2 id="데코레이터-팩토리-decorator-factories"/,
      ],
      [
        "ko/docs/handbook/enums.html",
        /<span id="런타임에서-열거형-enums-at-runtime" aria-hidden="true"><\/span><h2 id="런타임에서의-열거형-enums-at-runtime"/,
      ],
      [
        "ko/docs/handbook/enums.html",
        /<span id="역-매핑-Reverse-mappings" aria-hidden="true"><\/span><h3 id="역-매핑-reverse-mappings"/,
      ],
      [
        "ko/docs/handbook/namespaces.html",
        /<span id="table-of-contents" aria-hidden="true"><\/span><h1 id="첫-번째-단계-first-steps"/,
      ],
      [
        "ko/docs/handbook/release-notes/typescript-3-8.html",
        /<span id="jsdoc-property-modifiers" aria-hidden="true"><\/span><h2 id="jsdoc-프로퍼티-지정자-jsdoc-property-modifiers"/,
      ],
      [
        "pt/docs/handbook/jsdoc-supported-types.html",
        /<span id="jsdoc-property-modifiers" aria-hidden="true"><\/span><h3 id="modificadores-de-propriedades-jsdoc"/,
      ],
      [
        "ja/docs/handbook/jsdoc-supported-types.html",
        /<span id="jsdoc-property-modifiers" aria-hidden="true"><\/span><h3 id="jsdocプロパティ修飾子"/,
      ],
    ]

    for (const [relativePath, pattern] of localizedAssertions) {
      const localizedPath = new URL(relativePath, dist)
      if (fs.existsSync(localizedPath)) assert.match(fs.readFileSync(localizedPath, "utf8"), pattern)
    }
  }
)
