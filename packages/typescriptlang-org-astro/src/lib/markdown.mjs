...
import typescriptLightTheme from "../themes/typescript-beta-light.json" with { type: "json" }
  ...
  const visit = node => {
    if (node.type === "heading") {
      ...
      node.children.unshift({
        ...
        url: `#${id}`,
        title: null,
        data: { hProperties: { className: ["anchor", "before"], ariaLabel: `${title.toLowerCase()} permalink` } },
        children: [{
          type: "html",
...
const processMarkdown = async (markdown, options) => {
  ...
  return String(result).replace(/<div class=(['"])code-container\1>/g, '<div class="code-container" tabindex="0">')
}
