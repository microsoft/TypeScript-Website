...
const documentationNavigationArtifact = fs.existsSync(documentationNavigationPath) ? JSON.parse(fs.readFileSync(documentationNavigationPath, "utf8")) : {}
const documentationAttribution = fs.existsSync(documentationAttributionPath) ? JSON.parse(fs.readFileSync(documentationAttributionPath, "utf8")) : {}
...
const slug = value => value.toLowerCase().replace(/[^\x00-\x7F]/g, "-").replace(/[ /+]/g, "-")
...
const playgroundDetails = (code, filename) => {
  ...
  code.replace(/\r\n/g, "\n").split("\n").forEach((line, index, all) => {
    ...
    if (!comment && state === "comment") { state = "code"; seoLines.push("<code><pre>", line); return }
    if (comment && state === "code") { if (nextComment) { seoLines.push("</pre></code>"); state = "comment" }; seoLines.push(line); return }
    seoLines.push(line)
  })
  return { inlineTitle, query, prose: prose.join("\n").trim(), exampleCode: lines.join("\n").trim(), seoHtml: seoLines.join("\n") }
}

export function createRoutes() {
  if (routeCache) return routeCache
  const routes = []
  const rootPages = ["", "cheatsheets", "community", "download", "empty", "tools", "why-create-typescript", "docs", "docs/handbook", "dt/search"]
  const siteLocales = ["en", "es", "fr", "id", "ja", "ko", "pl", "pt", "vo", "zh"]
  for (const locale of siteLocales) for (const page of rootPages) {
    routes.push(route([locale === "en" ? "" : locale, page].filter(Boolean).join("/"), "root", { locale, title: page || "TypeScript" }))
  }
  ...
  for (const file of walk(path.join(packages, "documentation", "copy")).filter(file => file.endsWith(".md"))) {
    ...
    routes.push(route(String(parsed.data.permalink).replace(/\/+/g, "/"), "documentation", {
  ...
  const tsconfigOutputs = walk(path.join(packages, "tsconfig-reference", "output"))
    .filter(file => /^[a-z]{2}\.md$/.test(path.basename(file)))
  for (const file of tsconfigOutputs) {
    const locale = path.basename(file, ".md")
    routes.push(route(locale === "en" ? "tsconfig" : `${locale}/tsconfig`, "tsconfig", {
      locale, title: "TSConfig Reference", source: file, markdown: fs.readFileSync(file, "utf8"),
    }))
  }

  const glossary = path.join(packages, "glossary", "output", "en.md")
  routes.push(route("glossary", "glossary", { locale: "en", title: "TypeScript Glossary", source: glossary, markdown: fs.existsSync(glossary) ? fs.readFileSync(glossary, "utf8") : "" }))
  ...
  for (const locale of playgroundLocales) routes.push(route(locale === "en" ? "play" : `${locale}/play`, "playground", { locale, title: "TypeScript Playground" }))
  ...
  for (const file of walk(examplesRoot).filter(file => /\.(js|ts)$/.test(file))) {
    ...
    const id = path.basename(file).replace(/\.(js|ts)x?$/, "").toLowerCase().replace(/[^\x00-\x7F]/g, "-").replace(/[ /+]/g, "-")
    routes.push(route(`${locale === "en" ? "" : `${locale}/`}play/${postLocale}.html`, "playground-example", {
      locale, title: details.inlineTitle || path.basename(file).replace(/\.(js|ts)x?$/, ""), source: file, code,
      prose: details.prose, exampleCode: details.exampleCode, seoHtml: details.seoHtml,
  ...
  for (const file of walk(optionsRoot).filter(file => file.endsWith(".md"))) {
    ...
    routes.push(route(`tsconfig/${name}.html`, "tsconfig-option", {
      locale: "en", title: name, source: file, markdown: parsed.content, frontmatter: parsed.data, redirect: `/tsconfig#${name}`,
    }))
  }
  ...
  for (const file of walk(handbookRoot).filter(file => file.endsWith(".md"))) {
    routes.push(route(`_playground-handbook/${slug(path.basename(file, ".md"))}.html`, "playground-handbook", {
      locale: "en", title: path.basename(file, ".md"), source: file, markdown: fs.readFileSync(file, "utf8"),
    }))
  }

  for (const pathname of ["branding", "dev/bug-workbench", "dev/playground-plugins", "dev/sandbox", "dev/twoslash", "dev/typescript-vfs"])
...
const getDocumentationNavForLanguage = locale => documentationNavigationArtifact[locale] || documentationNavigationArtifact.en || []
...
const toDocumentationNavItems = (items, pathname) => items.map(item => ({
  title: item.title,
...
export function findDocumentationPageId(locale, pathname) {
  const current = findInNav(getDocumentationNavForLanguage(locale), item => item.permalink === normalizePathname(pathname))
  return current?.id
}
...
export function getDocumentationPrevNext(locale, pathname) {
  ...
  const section = findInNav(navs, item => item && !!item.items && !!item.items.find(subItem => subItem.id === currentId))
  ...
  return {
    previous: previousItem?.permalink ? { pathname: previousItem.permalink.replace(/^\//, ""), title: previousItem.title, description: previousItem.oneline } : undefined,
    next: nextItem?.items?.[0]?.permalink || nextItem?.permalink ? { pathname: (nextItem.items?.[0]?.permalink || nextItem.permalink).replace(/^\//, ""), title: nextItem.title, description: nextItem.oneline } : undefined,
  }
}
